...
void k_imageViewerTask(void) {
	...
	k_drawButton(windowId, &buttonArea, RGB(0, 0, 0), WINDOW_COLOR_BACKGROUND, "Show", BUTTON_FLAGS_SHADOW);
	...
	while (true) {
		...
		case EVENT_MOUSE_LBUTTONDOWN:
			...
			if (k_isPointInRect(&buttonArea, mouseEvent->point.x, mouseEvent->point.y) == true) {
				k_drawButton(windowId, &buttonArea, RGB(255, 255, 255), IMGVWR_COLOR_BUTTONACTIVE, "Show", BUTTON_FLAGS_SHADOW);
