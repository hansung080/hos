...
bool k_createPanel(Panel* panel, int x, int y, int width, int height, int itemSize, int itemMargin, Color* colors, dword flags) {
	...
	if (flags & PANEL_FLAGS_DRAWLOGO) {
		k_drawHosLogo(panel->id, (width - PANEL_LOGOWIDTH) / 2, ((height + WINDOW_SYSMENU_HEIGHT - PANEL_LOGOHEIGHT) / 2) - WINDOW_SYSMENU_HEIGHT, PANEL_LOGOWIDTH, PANEL_LOGOHEIGHT, PANEL_COLOR_LOGOBRIGHT, PANEL_COLOR_LOGODARK);
	}
