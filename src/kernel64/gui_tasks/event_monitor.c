...
void k_eventMonitorTask(void) {
	...
	k_drawButton(windowId, &buttonArea, RGB(0, 0, 0), WINDOW_COLOR_BACKGROUND, "Send User Event", BUTTON_FLAGS_SHADOW);
		...
		case EVENT_MOUSE_MBUTTONUP:
			...
			if (event.type == EVENT_MOUSE_LBUTTONDOWN) {
				if (k_isPointInRect(&buttonArea, mouseEvent->point.x, mouseEvent->point.y) == true) {
					...
					k_drawButton(windowId, &buttonArea, RGB(255, 255, 255), EVTMTR_COLOR_BUTTONACTIVE, "Send User Event", BUTTON_FLAGS_SHADOW);
