...
int main(const char* args) {
	...
	drawButton(windowId, &buttonArea, RGB(0, 0, 0), WINDOW_COLOR_BACKGROUND, "Send User Event", BUTTON_FLAGS_SHADOW);
		...
		case EVENT_MOUSE_OUT:
			...
			if (event.type == EVENT_MOUSE_LBUTTONDOWN) {
				if (isPointInRect(&buttonArea, mouseEvent->point.x, mouseEvent->point.y) == true) {
					...
					drawButton(windowId, &buttonArea, RGB(255, 255, 255), COLOR_BUTTONACTIVE, "Send User Event", BUTTON_FLAGS_SHADOW);
